...
from language import Language, LanguageImpl, tmpLanguage, defLanguage, \
                     parse, getOp, eval, evalstr
from macros import gensym, MacroContext, splice
...
from rootops import callOp, listOp, dictOp
# }}}

# {{{ globals
__noreload__ = ['lmodules']
...
def macroexpand1(x, context=None):
    if isinstance(x, Doc):
        ...
        return macros.expand1(x, language.getOp(x.tag), context)
    else:
        return x, False
# }}}
...
# {{{ def topython(x):
def topython(x):
    ast = logixcompiler.topy(macroexpand(x))
    try:
        import pyunparse
    except ImportError:
        return ast
    else:
        return pyunparse.ast2py(ast)        
# }}}

# {{{ def loadmodule(filename): DISABLED
...
def _imp(path, modulename, globals):
    ...
    if (not exists(os.path.join(path, basename + ".lx"))
        ...
        # {{{ modulename = full name including package
...
def execmodule(filename, env):
    ...
    code = baselang.__impl__.parse(file(filename, 'U'), filename,
                                   mode='execmodule', execenv=env)
    print "built %s in %.2fs" % (filename, time.time() - now)
