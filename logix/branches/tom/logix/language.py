    ...
    def parse(self, src, filename=None, execenv=None, mode='parse'):
        ...
        """ Modes:
            ...
            interactive = Expand macros, Eval language expressions only
                          (e.g. in switchlang), returns code-doc
                          (must supply an execenv)
            parse = Parse only, no macro expansion, no eval, returns code-doc
            expand = Parse and expand macros, no eval, returns code-doc
        ...
        try:
            if mode in ('exec', 'execmodule'):
                ...
                # {{{ parse TopLevelBlock, return code-doc list or codeobject
                ...
                # {{{ parse ExpressionRule, return code-doc (macro expanded)
                ...
                # {{{ parse BlockRule return code-doc (maybe macro expanded)
...
# {{{ def evalstr(language, src, globs=None, locs=None):
def evalstr(language, src, globs=None, locs=None):
    if globs is None and locs is None:
        frm = sys._getframe(1)
        locs  = frm.f_locals
    ...
    codedoc = parse(language, src)[0]

    return eval(codedoc, globs, locs)
# }}}
